...
<clsHMIActionName("Jump", enumHMIActionType.ManualAuto, enumHMISubActionType.SubAction)>
Public Class clsJumpAction
    ...
    Public Overrides Function Run(ByVal cLocalElement As Dictionary(Of String, Object), ByVal cSystemElement As Dictionary(Of String, Object), ByVal lListParameter As System.Collections.Generic.List(Of String)) As Boolean
                ...
                Select Case i.StepOutputNumber
                    ...
                    Case 1
                        Select Case lListParameter(1)
                            Case enumConditionMethod.Equal.ToString
                                i.StepInputNumber = 10
                            Case enumConditionMethod.NotEqual.ToString
                                i.StepInputNumber = 20
                        End Select

                    Case 10
                        If lListParameter(0) = lListParameter(2) Then
                            cActionResultCfg.Jump = True
                            If lListParameter(4) = "@END" Then
                                cActionResultCfg.JumpStep = Integer.MaxValue
                            Else
                                cActionResultCfg.JumpStep = lListParameter(3)
                            End If
                        Else
                            cActionResultCfg.Jump = False
                        End If
                    ...
                    Case 100
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("Jump", "1", GetMessage(cSubStepCfg))))
                        Return True

                    Case 200
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("Jump", "2", GetMessage(cSubStepCfg))))
                        i.StepInputNumber = i.StepInputNumber + 1

                    Case 201
                        cActionResultCfg.Result = False
                        cActionResultCfg.ErrorMessage = cLanguageManager.GetUserTextLine("Jump", "7", GetMessage(cSubStepCfg))
                        cActionResultCfg.ErrorType = enumJumpErrorType.JumpError.ToString
                        cActionResultCfg.MainErrorType = enumMainJumpErrorType.JumpError.ToString
                        cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("Jump", cActionResultCfg.ErrorType, 0)
                        Return False
                End Select
            Loop
            Return True
        Catch ex As Exception
            ...
            cActionResultCfg.ErrorType = enumJumpErrorType.UnKnownError.ToString
            cActionResultCfg.MainErrorType = enumMainJumpErrorType.JumpError.ToString
            cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("Jump", cActionResultCfg.ErrorType, 0)
            Return False
...
Public Enum enumMainJumpErrorType
    JumpError = 1
End Enum

Public Enum enumJumpErrorType
    JumpError = 1
    UnKnownError = 2
End Enum

Public Enum enumConditionMethod
    Equal = 1
    NotEqual
End Enum
