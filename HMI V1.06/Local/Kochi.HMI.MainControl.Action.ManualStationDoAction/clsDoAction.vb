...
<clsHMIActionName("ManualStationDoAction", enumHMIActionType.Manual, enumHMISubActionType.All)>
Public Class clsDoAction
    ...
    Public Overrides Function Run(ByVal cLocalElement As Dictionary(Of String, Object), ByVal cSystemElement As Dictionary(Of String, Object), ByVal lListParameter As System.Collections.Generic.List(Of String)) As Boolean
                ...
                Select Case i.StepOutputNumber
                    Case 0
                        ...
                        If lListParameter.Count > 2 Then
                            For j = 1 To lListParameter.Count - 1 Step 2
                                lListPic.Add(New clsPictureComponentCfg(lListParameter(j), lListParameter(j + 1)))
                            Next
                        End If
                        If lListPic.Count >= 1 Then
                            cPictureShowManager.ShowComponent(lListPic)
                        End If
                        If lListParameter(0) = "TRUE" Then
                    ...
                    Case 100
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("ManualStationDoAction", "3", GetMessage(cSubStepCfg))))
                        Return True

                    Case 200
                        cActionResultCfg.Result = False
                        cActionResultCfg.ErrorMessage = cLanguageManager.GetUserTextLine("ManualStationDoAction", "4", GetMessage(cSubStepCfg))
                        cActionResultCfg.ErrorType = enumDoActionErrorType.DoActionError.ToString
                        cActionResultCfg.MainErrorType = enumDoActionErrorType.DoActionError.ToString
                        cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("ManualStationDoAction", cActionResultCfg.ErrorType, 0)
                        Return False
                End Select
            Loop
            Return True
        Catch ex As Exception
            ...
            cActionResultCfg.ErrorType = enumDoActionErrorType.UnKnownError.ToString
            cActionResultCfg.MainErrorType = enumMainDoActionErrorType.DoActionError.ToString
            cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("ManualStationDoAction", cActionResultCfg.ErrorType, 0)
            Return False
        End Try
    End Function
...
Public Enum enumMainDoActionErrorType
    DoActionError = 1
End Enum

Public Enum enumDoActionErrorType
    DoActionError = 1
    UnKnownError = 2
End Enum