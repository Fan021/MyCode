...
<clsHMIActionName("ManualStationScrewBitClean", enumHMIActionType.Manual, enumHMISubActionType.All)>
Public Class clsScrewParkAction
    ...
    Public Overrides Function Run(ByVal cLocalElement As Dictionary(Of String, Object), ByVal cSystemElement As Dictionary(Of String, Object), ByVal lListParameter As System.Collections.Generic.List(Of String)) As Boolean
        ...
        Dim cIniHandler As clsIniHandler = CType(cSystemElement(clsIniHandler.Name), clsIniHandler)
        bExit = False
        ...
        Try
            ...
            If CInt(mTempValue) + 1 < CInt(lListParameter(1)) Then
                Dim iCnt As Integer = CInt(mTempValue) + 1
                cIniHandler.WriteIniFile(cSystemManager.Settings.ConfigFolder + "\" + "ScrewPart.ini", "Configure", "CurrentCount", iCnt.ToString)
                Return True
            Else
            ...
            Do While Not bExit
                ...
                Select Case i.StepOutputNumber
                    ...
                    Case 100
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("ManualStationScrewPark", "5", GetMessage(cSubStepCfg))))
                        Return True

                    Case 200
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("ManualStationScrewPark", "6", GetMessage(cSubStepCfg))))
                        i.StepInputNumber = i.StepInputNumber + 1

                    Case 201
                        cActionResultCfg.Result = False
                        cActionResultCfg.ErrorMessage = cLanguageManager.GetUserTextLine("ManualStationScrewPark", "6", GetMessage(cSubStepCfg))
                        cActionResultCfg.ErrorType = enumScrewParkErrorType.ScrewParkError.ToString
                        cActionResultCfg.MainErrorType = enumMainScrewParkErrorType.ScrewParkError.ToString
                        cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("ManualStationScrewPark", cActionResultCfg.ErrorType, 0)
                        Return False
                End Select
            Loop
            Return True
        Catch ex As Exception
            ...
            cActionResultCfg.ErrorType = enumScrewParkErrorType.UnKnownError.ToString
            cActionResultCfg.MainErrorType = enumMainScrewParkErrorType.ScrewParkError.ToString
            cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("ManualStationScrewPark", cActionResultCfg.ErrorType, 0)
            Return False
...
Public Enum enumMainScrewParkErrorType
    ScrewParkError = 1
End Enum

Public Enum enumScrewParkErrorType
    ScrewParkError = 1
    UnKnownError = 2
End Enum
