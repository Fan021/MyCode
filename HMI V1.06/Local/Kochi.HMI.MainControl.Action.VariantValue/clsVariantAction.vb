...
<clsHMIActionName("ActionVariantChange", enumHMIActionType.ManualAuto, enumHMISubActionType.SubAction)>
Public Class clsVariantAction
    ...
    Public Overrides Function Run(ByVal cLocalElement As Dictionary(Of String, Object), ByVal cSystemElement As Dictionary(Of String, Object), ByVal lListParameter As System.Collections.Generic.List(Of String)) As Boolean
        ...
        Try
            Do While Not bExit
                ...
                Select Case i.StepOutputNumber
                    ...
                    Case 1
                        If lListParameter(1) = "Bool" Then
                            Dim mTemp As String = cLocalVariantManager.CurrentVariantCfg.ListElement(lListParameter(0)).ToString
                            If mTemp = "" Then mTemp = "False"
                            cHMIPLC.WriteAny(lListParameter(2), Boolean.Parse(mTemp))
                        End If
                        If lListParameter(1) = "Int" Then
                            Dim mTemp As String = cLocalVariantManager.CurrentVariantCfg.ListElement(lListParameter(0)).ToString
                            If mTemp = "" Then mTemp = "0"
                            cHMIPLC.WriteAny(lListParameter(2), Int16.Parse(mTemp))
                        End If
                        If lListParameter(1) = "String" Then
                            Dim mTemp As String = cLocalVariantManager.CurrentVariantCfg.ListElement(lListParameter(0)).ToString
                            If mTemp = "" Then mTemp = ""
                            cHMIPLC.WriteAny(lListParameter(2), mTemp, New Integer() {mTemp.Length})
                        End If
                        cMainTipsManager.AddTips(New clsMainTipsManagerCfg(cRunnerCfg.StationName, cLanguageManager.GetUserTextLine("ActionVariantChange", "5", GetMessage(cSubStepCfg))))
                        Return True
        ...
        Catch ex As Exception
            ...
            cActionResultCfg.ErrorType = enumVariantChangeErrorType.UnKnownError.ToString
            cActionResultCfg.MainErrorType = enumMainVariantChangeErrorType.VariantChangeError.ToString
            cActionResultCfg.ErrorCode = cMachineManager.ActionParameterManager.GetActionParameterErrorCode("ActionVariantChange", cActionResultCfg.ErrorType, 0)
            Return False
        End Try
    End Function
...
Public Enum enumMainVariantChangeErrorType
    VariantChangeError = 1
End Enum

Public Enum enumVariantChangeErrorType
    VariantChangeError = 1
    UnKnownError = 2
End Enum
