...
Public Class clsProcessSFCManager
    ...
    Public Const Name As String = "ProcessSFCManager"
        ...
        SyncLock _Object
            Try
                ...
                For Each mDc As DataColumn In cDs_Data.Tables(0).Columns
                    mDc.ColumnName = cLanguageManager.GetUserTextLine("ProcessSFCManager", mDc.ColumnName)
                Next
    ...
    Private Function DeleteData() As Boolean
        SyncLock _Object
            Try
                ...
                strDelCmd = "DELETE FROM hmidatabase.processsfctable where StartTime < '" & Now.AddDays(-7).ToString("yyyy-MM-dd HH:mm:ss") & "'"
    ...
    Public Function CreateDB() As Boolean
        SyncLock _Object
            Try
                ...
                strCheckTable = " select * from hmidatabase.processsfctable"
                strCreateTable = "CREATE TABLE `HMIDatabase`.`processsfctable`" & _
    ...
    Public Function InSertData(ByVal strSFC As String, ByVal strStation As String, ByVal strStartTime As String) As Boolean
        SyncLock _Object
            Try
                ...
                strInserCmd = "insert into hmidatabase.processsfctable (`SFC`,`Station`, `StartTime`) values ('" & strSFC & "', '" & strStation & "', '" & strStartTime & "')"
    ...
    Public Function UpdateData(ByVal strSFC As String, ByVal strStation As String, ByVal strEndTime As String) As Boolean
        SyncLock _Object
            Try
                ...
                strUpdateCmd = "UPDATE hmidatabase.processsfctable set `Station`='" & strStation & "',`StartTime`='" & strEndTime & "' where SFC='" & strSFC & "'"
    ...
    Public Function DeleteData(ByVal strID As String) As Boolean
        SyncLock _Object
            Try
                cMySqlAdapter.connstr = GetInitCmd(strIP)
                Dim strDelCmd As String = String.Empty
                strDelCmd = "DELETE FROM hmidatabase.processsfctable where ID = '" & strID & "'"
                Return cMySqlAdapter.DeleteData(strDelCmd)
            Catch ex As Exception
                Throw New clsHMIException(ex, enumExceptionType.Crash)
                Return False
    ...
    Public Function DeleteSFC(ByVal strSFC As String) As Boolean
        SyncLock _Object
            Try
                cMySqlAdapter.connstr = GetInitCmd(strIP)
                Dim strDelCmd As String = String.Empty
                strDelCmd = "DELETE FROM hmidatabase.processsfctable where SFC = '" & strSFC & "'"
                Return cMySqlAdapter.DeleteData(strDelCmd)
            Catch ex As Exception
                Throw New clsHMIException(ex, enumExceptionType.Crash)
                Return False
    ...
    Public Function HasSFC(ByVal strSFC As String) As Boolean
        SyncLock _Object
            Try
                ...
                strInquiryCmd = "select * from hmidatabase.processsfctable where SFC='" + strSFC + "' order by ID desc"
    ...
    Public Function SelectToDataView(ByRef Ds As DataSet, ByVal ParamArray cListSearchContion() As String) As Boolean
        SyncLock _Object
            Try
                ...
                If strSearchCmd = "" Then
                    strInquiryCmd = "SELECT * FROM hmidatabase.processsfctable order by ID desc"
                Else
                    strInquiryCmd = "SELECT * FROM hmidatabase.processsfctable " & strSearchCmd & "  order by ID desc"
                End If
