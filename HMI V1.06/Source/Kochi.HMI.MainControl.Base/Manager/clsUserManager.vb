...
Public Class clsUserManager
    ...
    Public Function HasUserAndPassWord(ByVal strName As String, ByVal strPassWord As String) As Boolean
        SyncLock _Object
            Try
                If lUserList.Any(Function(e) e.Name = strName And e.Password = strPassWord) Then
                    Return True
                Else
                    Return False
                End If
    ...
    Public Function AutoLogin() As Boolean
        ...
        SyncLock _Object
            Try
                cCurrentUserCfg.Level = [Enum].Parse(GetType(enumUserLevel), cMachineManager.MachineGlobalParameter.GetGlobalParameter(clsHMIGlobalParameter.AutoLoginLevel))
    ...
    Public Function DeleteData(ByVal strID As String) As Boolean
        SyncLock _Object
            Try
                lUserList.RemoveAt(CInt(strID) - 1)
                ChangeID()
    ...
    Public Function SelectToDataView(ByVal cViewPageType As enumViewPageType, ByVal ParamArray cListSearchContion() As String) As Boolean
        SyncLock _Object
            Try
                ...
                dt.Columns.Add(cLanguageManager.GetTextLine("UserManager", "ID"))
                dt.Columns.Add(cLanguageManager.GetTextLine("UserManager", "Name"))
                dt.Columns.Add(cLanguageManager.GetTextLine("UserManager", "Password"))
                dt.Columns.Add(cLanguageManager.GetTextLine("UserManager", "Level"))
                For Each element As clsUserCfg In lUserList
...
Public Class clsUserData
    ...
    Public Function InSertData(ByVal strID As String, ByVal strName As String, ByVal strPassword As String, ByVal strLevel As String) As Boolean
        SyncLock _Object
            Try
                Return cIniHandler.SetAnyListToIni(cSystemManager.Settings.UserConfig, "User" + strID, New String() {"ID", "Name", "Password", "Level"},
    ...
    Public Function DeleteData(ByVal strID As String) As Boolean
        SyncLock _Object
            Try
                Return cIniHandler.RemoveSection(cSystemManager.Settings.UserConfig, "User" + strID)
            Catch ex As Exception
                Throw New clsHMIException(ex, enumExceptionType.Crash)
                Return False
            End Try
    ...
    Public Function LoadUser(ByRef lUserList As List(Of clsUserCfg)) As Boolean
        SyncLock _Object
            Try
                ...
                For Each element As Dictionary(Of String, Object) In cIniHandler.GetAnyListFromIni(cSystemManager.Settings.UserConfig, "User", New String() {"ID", "Name", "Password", "Level"})
                    ...
                    End If
                    If CType(element("Password"), String) <> clsXmlHandler.s_DEFAULT And CType(element("Password"), String) <> clsXmlHandler.s_Null Then
                        cUserElement.Password = CType(element("Password"), String)
                    End If
                    If CType(element("Level"), String) <> clsXmlHandler.s_DEFAULT And CType(element("Level"), String) <> clsXmlHandler.s_Null Then
    ...
    Public Function SaveUser(ByVal lUserList As List(Of clsUserCfg)) As Boolean
        SyncLock _Object
            Try
                cIniHandler.RemoveAllSection(cSystemManager.Settings.UserConfig, "User")
                For Each element As clsUserCfg In lUserList
