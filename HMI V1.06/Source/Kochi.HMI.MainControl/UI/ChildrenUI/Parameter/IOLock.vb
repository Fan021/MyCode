...
Public Class IOLock
    ...
    Private tempListObject As New List(Of Object)
    ...
    Public Function Init(ByVal cLocalElement As Dictionary(Of String, Object), ByVal cSystemElement As Dictionary(Of String, Object)) As Boolean
        ...
        PostTest_Type.HeaderText = cLanguageManager.GetTextLine(enumUIName.ChildrenSystemParameterForm.ToString, "Type")
        ...
        PostTest_Value.HeaderText = cLanguageManager.GetTextLine(enumUIName.ChildrenSystemParameterForm.ToString, "Value")
            ...
            Select Case element.TypeName
                ...
                Case "ProgramButton"
                    ...
                    If IsNothing(cIOCfg) Then
                        ...
                        element.Status = ""
                    Else
                        MachineListView_Lock.Rows.Add(MachineListView_Lock.Rows.Count + 1, cProgramButton.GetIOPageCfgFromID(element.IndexX).ActiveText, cIOCfg.ActiveText, element.Status)
                        MachineListView_Lock.Rows(MachineListView_Lock.Rows.Count - 1).Cells(1).Tag = element.IndexX
                    End If
                ...
                Case "ProgramCylinder"
                    ...
                    Else
                        If element.IndexY Mod 2 = 0 Then
                            MachineListView_Lock.Rows.Add(MachineListView_Lock.Rows.Count + 1, cProgramCylinderButton.GetCylinderPageCfgFromID(element.IndexX).ActiveText, cIOCfg.ActiveTextB, element.Status)
                        Else
                            MachineListView_Lock.Rows.Add(MachineListView_Lock.Rows.Count + 1, cProgramCylinderButton.GetCylinderPageCfgFromID(element.IndexX).ActiveText, cIOCfg.ActiveTextA, element.Status)
                        End If
                        MachineListView_Lock.Rows(MachineListView_Lock.Rows.Count - 1).Cells(1).Tag = element.IndexX + cIOManager.ListPage.Count
                    End If
        ...
        RemoveHandler MachineListView_Lock.CellEnter, AddressOf MachineListView_Lock.ListView_CellEnter
        Return True
    End Function
            ...
            Select Case MachineListView_Lock.Columns(e.ColumnIndex).Name
                Case "PostTest_Type"
                    ...
                    Else
                        ...
                        If iCnt > cIOManager.ListPage.Count And iCnt <= cIOManager.ListPage.Count + cProgramButton.ListPage.Count Then
                            For Each element As clsIOCfg In cProgramButton.ListPage(iCnt - cIOManager.ListPage.Count).ListIO.Values
                                tempListObject.Add(element.ActiveText)
                            Next
                            lListIO(e.RowIndex).TypeName = "ProgramButton"
                            lListIO(e.RowIndex).IndexX = iCnt - cIOManager.ListPage.Count
                        End If

                        If iCnt > cIOManager.ListPage.Count + cProgramButton.ListPage.Count Then
                            For Each element As clsCylinderCfg In cProgramCylinderButton.ListPage(iCnt - cIOManager.ListPage.Count - cProgramButton.ListPage.Count).ListIO.Values
                                tempListObject.Add(element.ActiveTextA)
                                tempListObject.Add(element.ActiveTextB)
                            Next
                            lListIO(e.RowIndex).TypeName = "ProgramCylinder"
                            lListIO(e.RowIndex).IndexX = iCnt - cIOManager.ListPage.Count - cProgramButton.ListPage.Count
                        End If
