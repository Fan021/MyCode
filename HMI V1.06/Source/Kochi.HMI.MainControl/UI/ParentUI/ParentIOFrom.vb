...
Public Class ParentIOFrom
    ...
    Public Function Init(ByVal cSystemElement As Dictionary(Of String, Object)) As Boolean Implements IParentUI.Init
        Try
            ...
            cLocalElement.Add(clsMainButtonManager.Name, cMainButtonManager)
            ...
            Select Case CType([Enum].Parse(GetType(enumHMI_LEFT_ITEM), strParentName), enumHMI_LEFT_ITEM)
                ...
                Case enumHMI_LEFT_ITEM.System
                    cMainButtonManager.RegisterManager(Panel_Mid,
                                          ...
                                          New clsMainButtonManagerCfg(TableLayoutPanel_Right_Item, "PLCMessage", cLanguageManager.GetTextLine(enumUIName.ParentSystemForm.ToString, "PLCMessage"), GetType(ChildrenPlcMessageListForm)), New clsMainButtonManagerCfg(TableLayoutPanel_Right_Item, "DeviceManager", cLanguageManager.GetTextLine(enumUIName.ParentSystemForm.ToString, "DeviceManager"), GetType(ChildrenDevicesManagerForm)),
    ...
    Public Function Quit(ByVal cSystemElement As Dictionary(Of String, Object)) As Boolean Implements IParentUI.Quit
        Try
            ...
            If Not IsNothing(cHMIPLC) Then
                Dim bDebugMode As Boolean = cHMIPLC.ReadAny(HMI_PLC_Interface.HMI_AutoMainFunction_AdsName + ".bulDebugMode", GetType(Boolean))
                If bDebugMode Then
                    cIOManager.LoadData()
                    Dim iPageNr As Integer = cIOManager.ListTypeNumber(enumIOType.EL1008)
                    If iPageNr <= 0 Then iPageNr = 1
                    If Not IsNothing(cHMIPLC) Then cHMIPLC.WriteAny(HMI_PLC_Interface.HMI_DI_EL1008_AdsName, New Boolean(iPageNr * HMI_PLC_Interface.CON_MAXIMUM_PageNumber) {})
                    iPageNr = cIOManager.ListTypeNumber(enumIOType.EP1008)
                    If iPageNr <= 0 Then iPageNr = 1
                    If Not IsNothing(cHMIPLC) Then cHMIPLC.WriteAny(HMI_PLC_Interface.HMI_DI_EP1008_AdsName, New Boolean(iPageNr * HMI_PLC_Interface.CON_MAXIMUM_PageNumber) {})
                    iPageNr = cIOManager.ListTypeNumber(enumIOType.EL2008)
                    If iPageNr <= 0 Then iPageNr = 1
                    If Not IsNothing(cHMIPLC) Then cHMIPLC.WriteAny(HMI_PLC_Interface.HMI_DO_EL2008_AdsName, New Boolean(iPageNr * HMI_PLC_Interface.CON_MAXIMUM_PageNumber) {})
                    ...
                    If Not IsNothing(cHMIPLC) Then cHMIPLC.WriteAny(HMI_PLC_Interface.HMI_Cylinder_AdsName, cDefaultValue)
