...
Public Class Laser
    ...
    Public Function Init(ByVal mType As LasDeviceType, ByVal mConfig As String, ByVal MyStation As Station, ByVal _AppSettings As Settings, ByVal MyLanguage As Language) As Boolean Implements ILaser.Init
        ...
        ElseIf mType = LasDeviceType.LF100_LAN Then
            ...
            _DeviceType = DeviceType.LAN
        ElseIf mType = LasDeviceType.MDX1000_LAN Then
            _iLaser = New Keyence_Laser_MDX1000()
            _DeviceType = DeviceType.LAN
        ElseIf mType = LasDeviceType.AF20_USB Then
            _iLaser = New EzdLaser
...
Public Class AlltecFoba_Laser_DP10GS
    ...
    'MDX1000_LAN
    Public Const _Separator As Char = ","c
    Public Const CMD_SETGA As String = "GA"
    Public Const CMD_GETFE As String = "FE"
    Public Const CMD_SETC2 As String = "C2"
    Public Const CMD_GETB3 As String = "B3"
    Public Const CMD_MDX1000_ACK As String = "0"
    Public Const CMD_MDX1000_GETSTATUS As String = "RE"
    Public Const CMD_MDX1000_Start As String = "RE"
    Public Const CMD_SETMARK As String = "NT"
    'LF100
...
#End Region
    ...
    Protected Overridable Sub a_SendVariable()

        _Status = Alltec_StatusCode.InSendStartMode
        _StatusDescription = _Status.ToString

        Dim cmd As String = CMD_SETVARS & _Delimiter & _cmdVariableName & _Delimiter &
...
Public Class Keyence_Laser_MDX1000
    Inherits AlltecFoba_Laser_DP10GS

    Public Sub New()
        MyBase.New()
        _LaserType = enumLaserType.MDX1000
    End Sub
    ...
    Public Overrides Sub Main()
        Do
            ...
            Select Case _StepCurrentNumber
                ...
                Case 3 : a_SendVariable()
                Case 4 : a_ReceivePositiveResponse()
                Case 5 : a_SendCommand(CMD_GETB3 & _Separator)
                Case 6 : a_ReceiveResponse()
                Case 7 : a_Ready()
                'set and get specific Template
                Case 10 : a_GetPing()
                ...
                Case 12 : a_SetTemplate()
                Case 13 : a_ReceivePositiveResponse()
                Case 14 : a_SendCommand(CMD_GETFE)
                Case 15 : a_TempleReceiveResponse()
                Case 16 : a_Ready()
                'set any command
                Case 20 : a_GetPing()
    ...
    Public Overrides Function GetStatus() As Boolean
        SetAnyCommand(CMD_MDX1000_GETSTATUS & vbCrLf)
        Return True
    End Function


    Public Overrides Function GetVar() As Boolean
        SetAnyCommand(CMD_GETB3 & vbCrLf)
        Return True
    End Function

    Public Overrides Function Start() As Boolean
        SetAnyCommand(CMD_MDX1000_Start & vbCrLf)
        Return True
    End Function

    Public Overrides Function GetGetTemplate() As Boolean
        SetAnyCommand(CMD_GETFE & vbCrLf)
        Return True
    End Function

    Public Overrides Function GetStatusReady(ByVal mTemplateName As String) As Boolean

        If Not LastResponse.IndexOf(Alltec_StatusCode.Ready) >= 0 Then
            _Status = Alltec_StatusCode.NotReady
            _StatusDescription = _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_LASER_ERROR2, _Status.ToString, _LastResponse)
            Return False
        End If
    ...
    Protected Overrides Sub a_SendVariable()

        _Status = Alltec_StatusCode.InSendStartMode
        _StatusDescription = _Status.ToString

        Dim cmd As String = CMD_SETC2 & _Separator & _cmdVariableName & _Separator &
                _cmdVariableValue & vbCrLf
        Dim _Buffer() As Byte = Encoding.UTF8.GetBytes(cmd)
    ...
    Protected Overrides Sub a_ReceiveResponse()
        ...
        Try
            ...
            Results = Split(_LastResponse, _Separator)
            If Results.GetUpperBound(0) < 2 Then
                _Status = Alltec_StatusCode.InvalidResponse
                _StatusDescription = _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_LASER_ERROR2, _Status.ToString, "")
                Abort()
    ...
    Protected Overrides Sub a_TempleReceiveResponse()
        ...
        Try
            ...
            _Interface.Receive(_Buffer)
            _NowReceived = _NowReceived & Encoding.ASCII.GetString(_Buffer)
            Results = Split(_NowReceived, vbCr)
            If Results.GetUpperBound(0) < 1 Then
                _LastResponse = "Error"
                Exit Sub
            End If
            ...
            _LastResponse = Results(0)
            Dim a As Integer = Results.GetUpperBound(0)
            Results = Split(_LastResponse, _Separator)
            If Results.GetUpperBound(0) < 1 Or _GetTempCount > 300 Then
                _Status = Alltec_StatusCode.InvalidResponse
                _StatusDescription = _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_LASER_ERROR2, _Status.ToString, "")
                Abort()
            ElseIf Results(0) = "" Then
            ...
            ElseIf Results(1).Length < 1 Then
                _Status = Alltec_StatusCode.InvalidResponse
                _StatusDescription = _Status.ToString
                Abort()
    ...
    Protected Overrides Sub a_SetTemplate()
        ...
        Dim cmd As String
        cmd = CMD_SETGA & _Separator & _cmdTemplateName & vbCrLf
        Dim _Buffer() As Byte = Encoding.UTF8.GetBytes(cmd)
