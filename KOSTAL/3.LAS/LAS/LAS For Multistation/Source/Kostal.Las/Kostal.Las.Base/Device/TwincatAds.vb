...
#Region "LAS_PLC_BaseAdsClass"
    ...
    Public Class KostalAdsVariables
        ...
        <AutomationVariable()> Public Const PLC_bulArticleDisable As String = CON_DOT & "PLC_bulArticleDisable"
...
Public Class structStationOverviewInfo
    ...
    <MarshalAs(UnmanagedType.I1, SizeConst:=1)> Public PLC_bulArticleDisable As Boolean = False
    ...
    Public Function Run() As Boolean
        ...
        Try
            ...
            For Each substation As IStationTypeBase In _Stations.Values
                ...
                'ForcamStation ADS请求
                If TypeOf substation Is ForcamStation Then
                    Dim station As ForcamStation = CType(substation, ForcamStation)
                    If station.SubStationCfg.AdsInput.Count < 1 Then Continue For
                    station.AdsValue = CType(GetDeviceNotificationEx(station.SubStationCfg.AdsInput(0)), Int16)
                End If
                ...
                'Mulit ADS请求
                If TypeOf substation Is MulitPrinter Then
                    Dim station As MulitPrinter = CType(substation, MulitPrinter)
                    If station.SubStationCfg.AdsInput.Count < 1 Then Continue For
                    Dim deciceSet As StructDeviceInteraction = CType(ReadChangePLCVersion(GetDeviceNotificationEx(station.SubStationCfg.AdsInput(0))), StructDeviceInteraction)
                    If Not station.LockArticle Then
                        station.ReadStructDeviceInteraction.stuPlcArticleSet.strKostalNr = deciceSet.stuPlcArticleSet.strKostalNr
                ...
                'UserDefineLineControl ADS请求
                If TypeOf substation Is UserDefineLineControlStation Then
                    Dim Station As UserDefineLineControlStation = CType(substation, UserDefineLineControlStation)
                    If Station.SubStationCfg.AdsInput.Count < 1 Then Continue For
                    Dim deciceSet As StructRequestAction = CType(ReadChangePLCVersion(GetDeviceNotificationEx(Station.SubStationCfg.AdsInput(0))), StructRequestAction)
                    If Station.SubStationCfg.AdsInput.Count > 1 And (deciceSet.bulDoNegativeAction Or deciceSet.bulDoPositiveAction) Then
                        Station.PLC_OUT_WT = ReadFailedPartInfo(Station.SubStationCfg.AdsInput(1))
        ...
        Catch ex As Exception
            If mTempstation IsNot Nothing Then
                _StatusDescription = MyLanguage.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_TWINCAT_ERROR1, mTempstation.SubStationCfg.Name, ex.Message.ToString)
            Else
                _StatusDescription = MyLanguage.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_TWINCAT_ERROR2, ex.Message.ToString)
            End If
            Return False
        End Try
        Return True
    ...
    Public Function GetDeviceNotificationEx(ByVal Name As String) As Object
        SyncLock _Object
            SyncLock _Object2
                ...
                Else
                    Throw New Exception("lDeviceNotificationEx not ContainsKey:" + Name)
                End If
                Return Nothing
            End SyncLock
        End SyncLock
        ...
        Try
            SyncLock _Object
                ...
                Try
                    ...
                    Return True
                Catch ex As Exception
                    _StatusDescription = MyLanguage.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_TWINCAT_ERROR11, name, ex.Message.ToString)
                    Return False
                End Try
            End SyncLock
        Catch ex As Exception
