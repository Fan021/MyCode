...
Public Class LineControlStation
    ...
    Protected _packman As PackmanStation
    ...
    Public Overrides Sub Run()
        Try
            ...
            Select Case _i.StepOutputNumber
                ...
                Case -97
                    If Not LineControlInit(_SubStationCfg.Config) Then
    ...
    Protected Function _a_Check_Packman() As Boolean
        If _SubStationCfg.Packman <> "" Then
            _packman = CType(_Stations(_SubStationCfg.Packman), PackmanStation)
            _i.StepInputNumber = _i.StepOutputNumber + 1
        Else
            _Logger.Logger(_i, _Messager, _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_TEXT_LINECONTROL_COUNTER, "Disable"))
            _i.StepInputNumber = _i.StepOutputNumber + 3
    ...
    Protected Function _a_Check_PackmanRun() As Boolean
        If Not _packman.isRun Then
            _packman.isRun = True
            _i.StepInputNumber = _i.StepOutputNumber + 1
        End If
        Return True
    ...
    Protected Function _a_Packman() As Boolean
        _packman.ReadStructRequestAction.stuPlcArticleSet = _ReadStructRequestAction.stuPlcArticleSet
        _packman.ReadStructRequestAction.strActionScheduleName = _ReadStructRequestAction.strActionScheduleName
        If _InternFail Then
            _packman.ReadStructRequestAction.bulDoNegativeAction = True
            _packman.ReadStructRequestAction.bulDoPositiveAction = False
        Else
            _packman.ReadStructRequestAction.bulDoNegativeAction = _ReadStructRequestAction.bulDoNegativeAction
            _packman.ReadStructRequestAction.bulDoPositiveAction = _ReadStructRequestAction.bulDoPositiveAction
        End If
        _Logger.Logger(_i, _Messager, "Packman Start")
        _i.StepInputNumber = _i.StepOutputNumber + 1
        Return True
    End Function

    Protected Function _a_Wait_Packman() As Boolean
        If IsNothing(_packman) Then
            _i.StepInputNumber = _i.StepOutputNumber + 1
            Return True
        End If
        If _packman.WriteStructResponseAction.bulActionIsPass Then
            _packman.ReadStructRequestAction.bulDoNegativeAction = False
            _packman.ReadStructRequestAction.bulDoPositiveAction = False
            _packman.WriteStructResponseAction.bulActionIsPass = False
            _packman.WriteStructResponseAction.bulActionIsFail = False
            _packman.WriteStructResponseAction.bulPartReceived = False
            _packman.isRun = False
            _Logger.Logger(_i, _Messager, _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_TEXT_LINECONTROL_COUNTER_END))
            _i.StepInputNumber = _i.StepOutputNumber + 1
        End If
        Return True
