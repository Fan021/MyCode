...
Public Class MesStation
    ...
    Protected WithEvents _Mes As New MES
    Public Const Name As String = "MesStation"
    ...
    Public Overrides Sub Run()
        Try
            ...
            If Not UpdateMsg(MesStation.Name) Then Return
            '==============================================================================
            ...
            Select Case _i.StepOutputNumber
                ...
                Case -99
                    If _SubStationCfg.Enable Then
                        If Not _Mes.Init(SubStationCfg.Station, SubStationCfg.Name, _i, AppSettings, _Language, SubStationCfg.Config) Then
                            _Logger.ThrowerNoStation(_i, _Messager, _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_MES_INIT_FAIL, "FAIL", _Mes.StatusDescription), "Mes.Init")
                        Else
                            _Logger.Logger(_i, _Messager, _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_MES_INIT_PASS, "PASS"), "Mes.Init")
                            _i.StepInputNumber = _i.StepOutputNumber + 1
                        End If
                    Else
                        If _i.Toggle Then
                            _Logger.Logger(_i, _Messager, _Language.LanguageElement.GetTextLine(_i, enumLK_TEXT.LK_MES_INIT_PASS, "Disable"), "Mes.Init")
                        End If
                    End If
                Case -98
                ...
                Case 18
                    _a_Write_Mes_Complete()

                Case 19
                    _a_Check_Mes_Complete_Result()
    ...
    Protected Sub _a_Mes_Start()
        _Mes.MesStart(_LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data, _PLC_OUT_WT.PartFailText)
        _i.StepInputNumber = _i.StepOutputNumber + 1
    End Sub


    Protected Sub _a_Check_Mes_Start_Result()
        If Not _Mes.MesStartStamp_RUN Then
            If _Mes.Status = enumMESStatus.StartPass Then
                '   _Logger.Logger(_i, _Messager, _Mes.StatusDescription)
                If _Mes.HasStart Then _UIStation.AddRow(
                       "Start",
                      ...
                      _LocalArticle.ArticleElements(KostalArticleKeys.KEY_ARTICLE_FAMILY).Data,
                       ...
                       "",
                      True,
                      _Mes.StatusDescription)
                _i.StepInputNumber = _i.StepOutputNumber + 1
            Else
                _Logger.Logger(_i, _Messager, _Mes.StatusDescription)
                _InternFail = True
                _InternPass = False
                _InternMsg = _Mes.StatusDescription
                If _Mes.HasStart Then _UIStation.AddRow(
                       "Start",
                      _LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data,
                      _LocalArticle.ArticleElements(KostalArticleKeys.KEY_ID).Data,
    ...
    Protected Sub _a_Write_Mes_Complete()
        If _InternFail Then
            _InternMsg = _InternMsg + ";" + _PLC_OUT_WT.PartFailText
            _i.StepInputNumber = _i.StepOutputNumber + 2
            '  _Mes.MesComplete(_LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data, Not _InternFail, _InternMsg)
        Else
            _Mes.MesComplete(_LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data, _ReadStructRequestAction.bulDoPositiveAction, _PLC_OUT_WT.PartFailText)
            _i.StepInputNumber = _i.StepOutputNumber + 1
        End If

    End Sub

    Protected Sub _a_Check_Mes_Complete_Result()
        If Not _Mes.MesCompleteStamp_RUN Then
            If _Mes.Status = enumMESStatus.CompletePass Then
                '   _Logger.Logger(_i, _Messager, _Mes.StatusDescription)
                If _Mes.HasComplete Then
                    _UIStation.AddRow(
                   "Complete",
                  _LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data,
                  _LocalArticle.ArticleElements(KostalArticleKeys.KEY_ID).Data,
            ...
            Else
                ...
                _InternPass = False
                _InternMsg = _Mes.StatusDescription
                If _Mes.HasComplete Then _UIStation.AddRow(
                   "Complete",
                  _LocalArticle.ArticleElements(KostalArticleKeys.KEY_SERIAL_NUMBER).Data,
                  _LocalArticle.ArticleElements(KostalArticleKeys.KEY_ID).Data,
                  _LocalArticle.ArticleElements(KostalArticleKeys.KEY_CUSTOMER_NUMBER).Data,
