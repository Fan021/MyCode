...
'此文件定义了一些工具类来保存文件夹路径和LAS.xml和root.xml中的一些配置参数
...
End Enum
'此类用来保存从LAS.xml中读取的GeneralInformation标签信息
Public Class MachineIdentifier
    ...
    Protected _PlasmaInterval As String = String.Empty
    ...
    Public Property PlasmaInterval() As String
        Get
            Return _PlasmaInterval
        End Get
        Set(ByVal value As String)
            _PlasmaInterval = value
        End Set
    End Property
End Class
'此类用来保存从LAS.xml中读出的PLCs标签相关配置信息
...
End Class
'此类用来保存从LAS.xml中读取的Article_Csvs标签信息，用来储存ArticleFile和MappingFile文件路径
...
Public Enum enumLineType
    ...
    MultiLine = 2
End Enum
'此类用来保存个Assembly下各文件夹路径，LAS.xml中Schedule_Csv标签信息
...
End Class
'此类用来读取root.xml和LAS.xml的信息，将其保存到各具体类中
Public Class SetSettings
    ...
    Protected Function ReadMachineIdentifier() As Boolean
        ...
        sResult = xmlHandler.GetSectionInformation(mSettings.ConfigFolder, mSettings.ConfigName, "GeneralInformation", "PlasmaInterval")
        If sResult = XmlHandler.s_DEFAULT Or sResult = XmlHandler.s_Null Then
            Throw New Exception("Invalid PlasmaInterval. Please Add " + mSettings.ConfigName + "--PlasmaInterval")
            Return False
        End If
        mSettings.MachineIdentifier.PlasmaInterval = sResult
        Return True
    End Function
