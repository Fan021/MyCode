...
Public Class ChildrenIOForm
    ...
    Public Function Init(ByVal Devices As Dictionary(Of String, Object), ByVal Stations As Dictionary(Of String, IStationTypeBase), ByVal _AppSettings As Settings) As Boolean
        Try
            ...
            cIOManager.Init(Devices, Stations, _AppSettings)
            cIOManager.LoadData()

            cCylinderManager = New clsCylinderManager
            cCylinderManager.Init(Devices, Stations, _AppSettings)
            cCylinderManager.LoadData()

            Dim sResult As String = ""
            sResult = _xmlHandler.GetSectionInformation(_AppSettings.ConfigFolder, _AppSettings.ConfigName, "GeneralInformation", "WtPLCName")
            ...
            If e.Button = MouseButtons.Left Then
                ...
                If TypeOf cIOCfg.IO Is OutputIO Then
                    ...
                    If cIOCfg.IOTriggerType = enumIOTriggerType.Toggle Then
                        ...
                        If Not cIOLockManager.CheckLockIO(cIOCfg.ListLockIO) Then Return
    ...
    Private Sub ResetIO(ByVal bResult As Boolean)
        ...
        Dim iPageNr As Integer = cIOManager.ListTypeNumber(enumIOType.EL1008)
        If iPageNr <= 0 Then iPageNr = 1
        If Not IsNothing(cHMIPLC) And bAutoReset Then cHMIPLC.WriteAny(KostalAdsVariables.HMI_EL1008, New Boolean(iPageNr * 8) {})

        iPageNr = cIOManager.ListTypeNumber(enumIOType.EP1008)
        If iPageNr <= 0 Then iPageNr = 1
        If Not IsNothing(cHMIPLC) And bAutoReset Then cHMIPLC.WriteAny(KostalAdsVariables.HMI_EP1008, New Boolean(iPageNr * 8) {})

        iPageNr = cIOManager.ListTypeNumber(enumIOType.EL2008)
        If iPageNr <= 0 Then iPageNr = 1
        If Not IsNothing(cHMIPLC) And bAutoReset Then cHMIPLC.WriteAny(KostalAdsVariables.HMI_EL2008, New Boolean(iPageNr * 8) {})
    ...
    Public Function Quit(ByVal cSystemElement As Dictionary(Of String, Object)) As Boolean
        ...
        Loop
        StopRefreshUI()
        For Each cIOPageCfg As clsIOPageCfg In cIOManager.ListPage.Values
            For Each element As clsIOCfg In cIOPageCfg.ListIO.Values
                RemoveHandler element.IO.MainButton.MouseDown, AddressOf MainButton_Click
                RemoveHandler element.IO.MainButton.MouseDown, AddressOf MainButton_MouseDown
                RemoveHandler element.IO.MainButton.MouseUp, AddressOf MainButton_MouseUp
